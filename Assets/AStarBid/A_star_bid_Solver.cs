...
public static class AStarBidSolver
{
    ...
    private static CubeState InitializeCubeState(string state)
    {
        CubeState initialState = new CubeState
        {
            State = state,
            GCost = 0,
            HCost = AStarSolver.CalculateHeuristic(state),
            Parent = null
        };
    ...
    private static IEnumerable<CubeState> GetSuccessors(CubeState current)
    {
        ...
        {
            string newState = AStarSolver.ApplyMove(current.State, move);
            CubeState successor = new CubeState
            {
                State = newState,
                GCost = current.GCost + 1,
                HCost = AStarSolver.CalculateHeuristic(newState),
                Parent = current,
                Move = move
            };
